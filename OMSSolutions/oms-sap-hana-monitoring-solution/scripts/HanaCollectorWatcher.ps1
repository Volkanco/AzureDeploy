...
$ex=$null
  	...
  	Foreach($ins in $hanaconfig.HanaConnections.Rule)
	{
        ...
        $params1 = @{"collecttableinv" = $false;"configfolder" = "C:\HanaMonitor";"debuglog"=$false;"useManagedIdentity"=$false;"runmode"="default"}
		
        Start-AutomationRunbook `
		    -RunOn HANAMonitorGroup2 `
		    -Name AzureSAPHanaCollectorSingleDB `
		    -Parameters $params1
	...
	$colend=Get-date
	write-output "Finished trigger job in  $(($colend-$colstart).Totalseconds)  seconds"
