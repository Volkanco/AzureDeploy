...
[Parameter(Mandatory=$false)] [bool] $runconfigchecks=$false,
[Parameter(Mandatory=$false)] [bool] $runtoptables=$false,
[Parameter(Mandatory=$false)] [string] $configchecktype="AllExceptTables"
)
     ...
     If($runconfigchecks)
                {
                    ...
                    Write-output "Starting config check : 	$([Math]::Round($stopwatch.Elapsed.TotalSeconds,0)) seconds "


$query=$null

                IF( $configchecktype  -eq "Tables")
                {                        
                    $query="/* OMS -Query70 */WITH  
TEMP_INDEXES AS
( SELECT
    *
  FROM
...
( SELECT
  ...
  FROM
/* TMC_GENERATION_START_1  ONLY  TABLE CHECKS AND 'COLUMN_UNLOAD_SIZE' and  'LARGE_MEMORY_LOBS' */
    ( SELECT
        'MANY_PARTITIONS',
        '',
        TO_VARCHAR(COUNT(*))
    ...
    UNION ALL
    --437,'MEMORY', 'COLUMN_UNLOAD_SIZE',           'Size of unloaded columns (GB last day )', 
    ( SELECT
        'COLUMN_UNLOAD_SIZE',
        IFNULL(U.HOST, ''),
    ...
    UNION ALL
    --450,'MEMORY', 'LARGE_MEMORY_LOBS',            'Tables with memory LOBs > 2 GB
    ( SELECT
        'LARGE_MEMORY_LOBS',
        '',
...
##################################################################
                ...
                Elseif($configchecktype -eq "All")
                {

                $query="/* OMS -Query70 */WITH  TEMP_INDEXES AS
( SELECT
    *
  FROM
    INDEXES  --103288 Query run time: 	2 seconds 
...
( SELECT
    *
  FROM
    M_BACKUP_CATALOG_FILES  --189512 Query run time: 	7 seconds 
...
( SELECT
    *
  FROM
    M_CS_ALL_COLUMNS--too long
),
TEMP_M_CS_TABLES AS
( SELECT
    *
  FROM
    M_CS_TABLES  --97563 Query run time: 	72 seconds 
...
####################################################################
                ...
                {
                # We will run all except tables  -  most performant query type 
                Write-output "Running config checks excluding TABLES group"
                $query="/* OMS -Query70 */WITH  
TEMP_INDEXES AS
( SELECT     *   FROM     INDEXES ),
TEMP_M_BACKUP_CATALOG AS
...
$ex=$null
			...
			Catch
			{
				$Ex=$_.Exception.MEssage;write-warning "Failed to run Query70"
			}
 ...
 IF($runtoptables)
 {
        Write-Output "$((get-date).ToString('dd-MM-yyyy hh:mm:ss')) Query7 CollectorType=Inventory - Category=Tables - Largest"	

	  $query="/* OMS -Query71*/SELECT * FROM M_CS_TABLES  WHERE TABLE_NAME IN (SELECT TOP 50 TABLE_NAME FROM ( SELECT TABLE_NAME, SUM(RECORD_COUNT) as TOTAL FROM M_TABLES  GROUP BY TABLE_NAME )ORDER by TOTAL DESC)"
        $ds=$null
	   $cmd=new-object Sap.Data.Hana.HanaDataAdapter($Query, $conn);
		  $ds=New-Object system.Data.DataSet ;
	  $ex=$null
	    Try{
		  ...
		  Catch
		  {
			  $Ex=$_.Exception.MEssage;write-warning "Failed to run Query71"
			  write-warning  $ex 
		  }
