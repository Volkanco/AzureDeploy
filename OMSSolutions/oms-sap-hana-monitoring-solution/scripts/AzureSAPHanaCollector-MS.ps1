...
Write-output "IsHourlyschedule :$hourlySchedule"
Write-output "HasMiniChecksRan: $saphost  - $(($trackhost|where {$_.host -eq $saphost}).ConfigCheckRun)"
IF($hourlySchedule -and [string]::isnullorempty(($trackhost|where{$_.host -eq $saphost}).ConfigCheckRun) )
{
...
$ex=$null
			...
			$query='/* OMS */ Select t1.host,round(sum(t1.Total_memory_used_size/1024/1024/1024),1) as "UsedMemoryGB",round(sum(t1.physical_memory_size/1024/1024/1024),2) "DatabaseResident" ,SUM(T2.Peak) as PeakGB from m_service_memory  as T1 
Join 
...
# check takes long time to calculate
			...
			$query='/* OMS */ Select  host,schema_name ,sum(DISTINCT_COUNT) RECORD_COUNT,
...
#ystemReplication_Status
write-output "CollectorType=`"Inventory`" - Category=`"Replication_Status`" "

$query="/* OMS */ SELECT   R.SITE_NAME ,R.SECONDARY_SITE_NAME,R.HOST,R.SECONDARY_HOST,
...
$query="/* OMS */ SELECT  SNAPSHOT_TIME,
  HOST,
...
"
	...
	$cmd=new-object Sap.Data.Hana.HanaDataAdapter($Query, $conn);
			...
			Write-Output ' CollectorType="Inventory" ,  Category="TableLocation"'
