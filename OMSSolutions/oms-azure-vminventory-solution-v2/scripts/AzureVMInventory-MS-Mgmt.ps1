...
#get load balancer 
	...
	if($invVHDs.count -eq 0)
	{
		$invVHDs+=New-Object PSObject -Property @{
            ...
            VmName="NO RESOURCE FOUND"
            DiskIOPs="NO RESOURCE FOUND"
            DiskIOType="NO RESOURCE FOUND"
			SubscriptionId = $subscriptionID
			AzureSubscription = $subscriptionname
			ShowinDesigner=0
		}
	...
	if($invNSGs.count -eq 0)
	{
		$invNSGs+= New-Object PSObject -Property @{
            ...
            NSGName="NO RESOURCE FOUND"
            protocol="NO RESOURCE FOUND"
            destinationPortRange="NO RESOURCE FOUND"
			sourcePortRange="NO RESOURCE FOUND"
            sourceAddressPrefix="NO RESOURCE FOUND"
            destinationAddressPrefix="NO RESOURCE FOUND"
			SubscriptionId = $subscriptionID
			AzureSubscription = $subscriptionname
			ShowinDesigner=0
...
Write-verbose "After Runspace creation  $([System.gc]::gettotalmemory('forcefullcollection') /1MB) MB"
